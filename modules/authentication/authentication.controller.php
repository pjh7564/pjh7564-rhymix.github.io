...
class authenticationController extends authentication 
{
	...
	function procAuthenticationSendAuthCode()
	{
		...
		{		 
			...
			if($output->data->count > 0)
			{
				...
				foreach($output->data as $k => $v)
				{
					...
					if($member_info) return new baseObject(-1, '가입하신 휴대폰 번호로 중복 가입이 불가능합니다.');
				}
			}
		}
		...
		{
			return new baseObject(-1, '잦은 인증번호 요청으로 금지되셨습니다. 1일뒤에 다시 시도해주십시오.');
		}
		...
		if($output->data->count > 0)
		{
			return new baseObject(-1, $config->authcode_time_limit . '초 동안 다시 받으실 수 없습니다. 전송확인 버튼을 눌러 수신받지 못하는 사유를 확인하세요.');
		}
	...
	function procAuthenticationVerifyAuthCode()
	{
		...
		else
		{
			...
			$trigger_output = ModuleHandler::triggerCall ('authentication.procAuthenticationVerifyAuthCode', 'after', $reqvars);
			if(!$trigger_output->toBool ()) return $trigger_output;
			return new baseObject(-1,'인증코드가 올바르지 않습니다.');
		}
	}
	...
	function triggerMemberInsertBefore(&$in_args)
	{
		...
		if(!in_array("dispMemberSignUpForm", $action_list)) return new baseObject();
	...
	{
		if(!$in_args->member_srl) return new baseObject(-1, 'msg_invalid_request');
		$args->member_srl = $in_args->member_srl;
		$output = executeQuery('authentication.deleteAuthenticationMember', $args);
		if(!$output->toBool()) return $output;
	}
