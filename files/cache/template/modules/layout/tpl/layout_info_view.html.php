...
<section id="layout_config" class="x_form-horizontal">
	...
	<?php if($__Context->selected_layout->author){ ?><div class="x_control-group">
		...
		<div class="x_controls" style="padding-top:5px">
			<?php $__loop_tmp=$__Context->selected_layout->author;if($__loop_tmp)foreach($__loop_tmp as $__Context->author_info){ ?>
	...
	<?php if(!empty((array)$__Context->selected_layout->extra_var)){ ?><section class="section">
		...
		<?php $__Context->cnt = 1 ?>
		<?php $__loop_tmp=$__Context->selected_layout->extra_var;if($__loop_tmp)foreach($__loop_tmp as $__Context->name=>$__Context->var){ ?>
...
echo $__Context->var->value;
					...
					<?php if($__Context->var->type == 'image'){ ?>
						...
						<div id="preview_<?php echo $__Context->name ?>" class="x_thumbnail" style="<?php if(!$__Context->var->value){ ?>display:none;<?php } ?>max-width:210px;margin-bottom:20px;">
							<img<?php if($__Context->var->value){ ?> src="/dev/<?php echo $__Context->var->value ?>"<?php } ?> alt="" style="max-width:100%" />
					...
					<?php if($__Context->var->type == 'select'){ ?><select name="<?php echo $__Context->name ?>" id="<?php echo $__Context->name ?>" style="width:auto">
						<?php $__loop_tmp=$__Context->var->options;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->key ?>"<?php if($__Context->key == $__Context->var->value){ ?> selected="selected"<?php } ?>><?php echo $__Context->val->val ?></option><?php } ?>
					</select><?php } ?>
						...
						<div class="x_thumbnails">
							<?php $__loop_tmp=$__Context->var->options;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?>
								<div class="x_span2 <?php if($__Context->val->thumbnail){ ?>x_thumbnail<?php } ?>">
									<?php if($__Context->val->thumbnail){ ?><img src="/dev/<?php echo $__Context->val->thumbnail ?>" alt="<?php echo $__Context->val->val ?>" /><?php } ?>
									<div class="x_caption">
	...
	<section class="section">
		...
		<?php if($__Context->selected_layout->menu){ ?><div class="x_form-horizontal">
			<?php $__loop_tmp=$__Context->selected_layout->menu;if($__loop_tmp)foreach($__loop_tmp as $__Context->menu_name=>$__Context->menu_info){ ?><div class="x_control-group">
				...
				<div class="x_controls">
					<select name="<?php echo $__Context->menu_name ?>" id="<?php echo $__Context->menu_name ?>">
						...
						<?php $__loop_tmp=$__Context->menu_list;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->val->menu_srl ?>"<?php if($__Context->val->menu_srl == $__Context->menu_info->menu_srl){ ?> selected="selected"<?php } ?>><?php echo $__Context->val->title ?></option><?php } ?>
					</select>
				</div>
			</div><?php } ?>
		</div><?php } ?>
