...
<form id="fo_category_info" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
...
<table class="x_table x_table-striped x_table-hover">
	...
	<tbody>
		...
		<tr>
			<td colspan="3"><?php echo $lang->msg_category_is_null ?></td>
		</tr>
		<?php } ?>
		...
		<tr>
			...
			<td>
				<a class="x_btn x_btn-link" href="<?php echo getUrl('module_category_srl',$__Context->val->module_category_srl) ?>"><?php echo $lang->cmd_modify ?></a> 
				<i>|</i> 
				<button type="button" class="x_btn x_btn-link" onclick="doUpdateCategory('<?php echo $__Context->val->module_category_srl ?>','<?php echo $lang->confirm_delete ?>');return false;"><?php echo $lang->cmd_delete ?></button>
			</td>
		</tr>
		<?php } ?>
	</tbody>
...
<!-- 카테고리 추가 -->
<?php Context::addJsFile("modules/module/ruleset/insertCategory.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_clearfix"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" /><input type="hidden" name="ruleset" value="insertCategory" />
	<input type="hidden" name="module" value="module" />
	...
	<div class="x_pull-right x_input-append">
		<input type="text" name="title" title="<?php echo $lang->category_title ?>" placeholder="<?php echo $lang->category_title ?>" required />
