...
<section class="section">
	...
	<form action="./" class="x_form-horizontal" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<input type="file" name="addfile" id="file" data-max-filesize="<?php echo $__Context->max_filesize ?>" data-max-filesize-error="<?php echo htmlspecialchars(lang('file.allowed_filesize_exceeded'), ENT_QUOTES, 'UTF-8', true) ?>" />
			</div>
		</div>
		<div class="btnArea">
...
<section class="section">
	...
	<table class="x_table x_table-striped x_table-hover">
		...
		<tbody>
			<?php $__loop_tmp=$__Context->filebox_list;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?><tr>
				<td>
...
echo $__Context->val->filename ?>"><?php echo basename($__Context->val->filename) ?></a>
				...
				<td>
					<?php $__loop_tmp=$__Context->val->attributes;if($__loop_tmp)foreach($__loop_tmp as $__Context->name=>$__Context->value){ ?><p>
	...
	<?php if($__Context->page_navigation){ ?><form action="./" class="x_pagination x_pagination-centered" ><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
