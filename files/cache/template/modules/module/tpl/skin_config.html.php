			...
			<div class="x_controls">
				<?php $__loop_tmp=$__Context->skin_info->colorset;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?>
					...
					<div<?php if($__Context->val->screenshot){ ?> class="x_thumbnail"<?php } ?> style="display:inline-block;width:<?php echo $__Context->_width ?>px;margin-right:10px;">
						...
						<?php if($__Context->val->screenshot){ ?><img src="/dev/<?php echo $__Context->val->screenshot ?>" alt="<?php echo $__Context->val->title ?>" /><?php } ?>
					</div>
				<?php } ?>
			</div>
		</div><?php } ?>
...
$__loop_tmp=$__Context->val->options;if($__loop_tmp)foreach($__loop_tmp as $__Context->k=>$__Context->v){ ?><label for="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>" class="x_inline"><input type="checkbox" name="<?php echo $__Context->val->name ?>[]" value="<?php echo $__Context->v->value ?>" id="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>"<?php if(is_array($__Context->val->value) && in_array($__Context->v->value, $__Context->val->value)){ ?> checked="checked"<?php } ?> class="checkbox" /> <?php echo $__Context->v->title ?></label><?php }} ?>
