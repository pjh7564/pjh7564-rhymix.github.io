...
<table id="advanced_mailer_log" class="x_table x_table-striped x_table-hover">
	...
	<thead>
		<tr>
			...
			<th scope="col" class="nowr"><?php echo $lang->cmd_advanced_mailer_status_subject ?></th>
	...
	<tbody>
		<?php $__loop_tmp=$__Context->advanced_mailer_log;if($__loop_tmp)foreach($__loop_tmp as $__Context->mail_id=>$__Context->val){ ?><tr>
			<td class="nowr">
				<?php if($__Context->val->mail_from)foreach($__Context->val->mail_from as $__Context->no => $__Context->mail_from){ ?>
				<span title="<?php echo htmlspecialchars($__Context->mail_from[1]) ?>"><?php echo htmlspecialchars($__Context->mail_from[0]) ?></span><br />
				<?php } ?>
			</td>
			<td class="nowr">
				<?php if($__Context->val->mail_to)foreach($__Context->val->mail_to as $__Context->no => $__Context->mail_to){ ?>
				<span<?php if($__Context->no > 0){ ?> class="hidden"<?php } ?> title="<?php echo htmlspecialchars($__Context->mail_to[1]) ?>"><?php echo htmlspecialchars($__Context->mail_to[0]) ?></span>
				<?php if($__Context->no == 0 && count($__Context->val->mail_to) > 1){ ?><span class="show-hidden-recipients">+<?php echo count($__Context->val->mail_to) - 1 ?></span><?php } ?>
				<br />
				<?php } ?>
			</td>
			<td class="nowr" style="white-space:normal"><?php echo htmlspecialchars($__Context->val->subject) ?></td>
			<td class="nowr">
