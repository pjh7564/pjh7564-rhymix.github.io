...
<h1><?php echo $lang->msg_rechecked_password ?></h1>
...
<p><?php echo $lang->about_rechecked_password ?></p>
<?php Context::addJsFile("modules/member/ruleset/recheckedPassword.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="./index.php" method="post" ><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" /><input type="hidden" name="ruleset" value="recheckedPassword" />
	...
	<div>
		<input type="email" value="<?php echo $__Context->identifierValue ?>" readonly title="<?php echo $lang->email ?>" />
	</div>
	<span class="input-append">
		<input type="password" name="password" required placeholder="<?php echo $lang->password ?>" title="<?php echo $lang->password ?>" />
		<input type="submit" value="<?php echo $lang->cmd_confirm ?>" class="btn btn-inverse" />
	</span>
</form>
