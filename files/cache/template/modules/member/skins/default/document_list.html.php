...
<h1><?php echo $__Context->member_title = $lang->cmd_view_own_document  ?></h1>
<table class="table table-striped table-hover">
	...
	<thead>
		<tr>
			...
			<th><?php echo $lang->voted_count ?></th>
		</tr>
	</thead>
	<tbody>
		<?php $__loop_tmp=$__Context->document_list;if($__loop_tmp)foreach($__loop_tmp as $__Context->no=>$__Context->oDocument){ ?><tr>
			...
			<td class="title">
				<a href="<?php echo getUrl('','document_srl',$__Context->oDocument->document_srl) ?>" target="_blank"><?php echo $__Context->oDocument->getTitleText() ?></a>
...
<div class="pagination">
	<form action="<?php echo Context::getRequestUri() ?>" method="get"  style="float:left">
		...
		<select name="search_target" title="<?php echo $lang->search_target ?>">
			<option value="title"<?php if(isset($__Context->search_target) && $__Context->search_target == 'title'){ ?> selected="selected"<?php } ?>><?php echo $lang->title ?></option>
			<option value="title_content"<?php if(isset($__Context->search_target) && $__Context->search_target == 'title_content'){ ?> selected="selected"<?php } ?>><?php echo $lang->title_content ?></option>
			<option value="content"<?php if(isset($__Context->search_target) && $__Context->search_target == 'content'){ ?> selected="selected"<?php } ?>><?php echo $lang->content ?></option>
		</select>
