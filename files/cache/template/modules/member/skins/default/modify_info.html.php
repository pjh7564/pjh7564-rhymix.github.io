...
<!--#Meta:modules/member/tpl/js/signup_check.js--><?php Context::loadFile(['modules/member/tpl/js/signup_check.js', '', '', '']); ?>
<!--#JSPLUGIN:ui--><?php Context::loadJavascriptPlugin('ui'); ?>
<!--#JSPLUGIN:ui.datepicker--><?php Context::loadJavascriptPlugin('ui.datepicker'); ?>
<h1 style="border-bottom:1px solid #ccc"><?php echo $lang->msg_update_member ?></h1>
...
<?php Context::addJsFile("./files/ruleset/insertMember.xml", FALSE, "", 0, "body", TRUE, "") ?><form  id="fo_insert_member" action="./" method="post" enctype="multipart/form-data" class="form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" /><input type="hidden" name="ruleset" value="@insertMember" />
	...
	<div class="control-group">
		...
		<div class="controls" style="padding-top:5px">
			<?php $__loop_tmp=$lang->allow_message_type;if($__loop_tmp)foreach($__loop_tmp as $__Context->key=>$__Context->val){ ?><label for="allow_<?php echo $__Context->key ?>"><input type="radio" name="allow_message" value="<?php echo $__Context->key ?>"<?php if($__Context->member_info->allow_message == $__Context->key){ ?> checked="checked"<?php } ?> id="allow_<?php echo $__Context->key ?>" /> <?php echo $__Context->val ?></label><?php } ?>
		</div>
	</div>
	<div class="btnArea" style="border-top:1px solid #ccc;padding-top:10px">
		<input type="submit" value="<?php echo $lang->cmd_registration ?>" class="btn btn-inverse pull-right" />
		<a href="<?php echo getUrl('act','dispMemberInfo','member_srl','') ?>" class="btn pull-left"><?php echo $lang->cmd_cancel ?></a>
	</div>
</form>
<script>
jQuery(function($){
