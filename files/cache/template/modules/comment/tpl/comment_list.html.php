...
<form id="fo_list" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?? ''; ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
...
if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'Y'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
		...
		<tbody>
			<?php $__loop_tmp=$__Context->comment_list;if($__loop_tmp)foreach($__loop_tmp as $__Context->no=>$__Context->val){ ?><tr>
...
echo $__Context->comment;
				...
				<td class="nowr rx_detail_marks"><?php echo $__Context->val->getStatusText() ?></td>
...
<form action="./" method="get" class="search center x_input-append" onsubmit="return checkSearch(this)"><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
	<input type="hidden" name="module" value="<?php echo $__Context->module ?>" />
	<input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
	...
	<select name="search_target" title="<?php echo $lang->search_target ?>" style="margin-right:4px">
		<?php if(lang('comment.search_target_list'))foreach(lang('comment.search_target_list') as $__Context->key => $__Context->val){ ?>
...
<?php Context::addJsFile("modules/comment/ruleset/deleteChecked.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_modal x" id="listManager"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" /><input type="hidden" name="ruleset" value="deleteChecked" />
